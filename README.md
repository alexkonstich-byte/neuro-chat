...
Безопасная переписка в духе Telegram, плюс магазин кастомизации, казино со слотами
и Allsafe Premium — за свои собственные баллы, без покупок реальных денег.

> Раньше проект назывался **Neuro** — внутренние идентификаторы (БД, PM2, репо) кое-где сохранили старое имя для обратной совместимости.
...
- 🔖 **Избранное и Allsafe-бот** — автоматически создаются для каждого. В чат Allsafe идут коды входа и заявки в поддержку.
...
- ✦ **Allsafe Premium** — цвет ника, цвет префикса, кастомный эмодзи, эксклюзивные косметики.
...
│   │   └── service.js# системный бот Allsafe, Saved chat, login alerts
...
- **Нашёл баг или хочешь фичу?** Зайди в чат **Allsafe** внутри приложения — там кнопки «Баг», «Идея», «Сообщение админу». Заявки попадают в админ-инбокс автора.
...
**Alex Serguntsov** — делает Allsafe как личный домашний мессенджер.
